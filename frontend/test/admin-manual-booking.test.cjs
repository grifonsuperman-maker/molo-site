...
assert.match(planner, /Телефон гостя \(необов’язково\)/);
assert.match(planner, /placeholder="\+380 \(__\) ___-__-__"/);
assert.match(planner, /phone: phone \|\| undefined/);
assert.doesNotMatch(planner, /!manualPhone\.trim\(\)/);
assert.match(planner, /Офіціант побачить його у своєму пульті тільки в день бронювання/);
assert.match(api, /phone\?: string/);
