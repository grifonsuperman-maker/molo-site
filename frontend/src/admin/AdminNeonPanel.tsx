...
import AdminRestaurantMap from './AdminRestaurantMap';
import {
  ACTIVE_STATUSES,
  DateButton,
  EmptyState,
...
type Section = 'bookings' | 'map' | 'tables' | 'blacklist';
...
export default function AdminNeonPanel() {
  ...
  const activeCount = useMemo(
    () => sortedBookings.filter((booking) => ACTIVE_STATUSES.has(booking.status) && !isNoShow(booking)).length,
    [sortedBookings],
  );
  const tables = useMemo(
    () => [...(map?.tables || [])]
      .filter((table) => table.isVisible !== false)
  ...
  const futureDates = useMemo(
    () => (calendar?.dates || []).filter((item) => item.date >= today).slice(0, 14),
    [calendar, today],
  );
  const blacklisted = useMemo(() => {
    ...
    return clients
      ...
      .filter((client) => !query || `${client.fullName} ${client.phone}`.toLowerCase().includes(query));
  ...
  async function runBookingAction(booking: Booking, action: BookingAction) {
    ...
    try {
      ...
      setNotice(({
        ...
        checkIn: 'Гостя відмічено, стіл зайнятий',
        cleaning: 'Стіл переведено у підготовку',
  ...
  async function toggleBlacklist(client: Client, nextValue: boolean) {
    ...
    try {
      if (nextValue) await clientsApi.blacklist(client.id);
      else await clientsApi.unblacklist(client.id);
  ...
  async function submitChangeTable(booking: Booking) {
    ...
    await changeBookingTableTo(booking, changeTableId);
    setChangeTableBookingId(null);
    setChangeTableId('');
  }

  async function changeBookingTableTo(booking: Booking, targetTableId: string) {
    ...
    try {
      await adminBookingsApi.changeTable(booking.id, targetTableId);
    ...
    <main className="mx-auto min-h-screen max-w-7xl bg-black px-3 pb-28 pt-3 text-white sm:px-5 lg:px-8">
      <header className="sticky top-[72px] z-30 rounded-[24px] border border-amber-200/20 bg-neutral-950/95 p-3 shadow-[0_0_42px_rgba(251,191,36,.08)] backdrop-blur-xl sm:top-[76px]">
        <div className="flex items-center justify-between gap-3">
          <div>
            <p className="text-[10px] font-bold uppercase tracking-[0.22em] text-amber-100/55">MOLO</p>
            <h1 className="mt-1 text-xl font-black">Пульт адміністратора</h1>
          </div>
          <NeonButton tone="amber" compact onClick={() => openManual()}>＋ Бронь телефоном</NeonButton>
        </div>
        <div className="mt-3 grid grid-cols-4 gap-2">
          <TabButton active={section === 'bookings'} onClick={() => setSection('bookings')}>Броні</TabButton>
          <TabButton active={section === 'map'} onClick={() => setSection('map')}>План</TabButton>
      ...
      {(notice || error) && (
        <div className={`mt-3 rounded-2xl border px-4 py-3 text-sm ${error ? 'border-red-300/35 bg-red-500/10 text-red-100' : 'border-emerald-300/35 bg-emerald-400/10 text-emerald-100'}`}>
          {error || notice}
        </div>
      )}
            ...
            {futureDates.length > 0 && (
              <div className="mt-3 flex gap-2 overflow-x-auto pb-1">
                {futureDates.map((item) => (
                  <button key={item.date} type="button" onClick={() => setSelectedDate(item.date)} className={`shrink-0 rounded-2xl border px-3 py-2 text-left transition active:scale-95 ${selectedDate === item.date ? 'border-violet-200/70 bg-violet-400/20 shadow-[0_0_24px_rgba(167,139,250,.22)]' : 'border-violet-300/20 bg-violet-400/5'}`}>
                    <p className="text-xs font-black text-violet-100">{formatDate(item.date)}</p>
                    <p className="mt-1 text-[10px] text-white/50">{item.total} броней · {item.guests} гостей</p>
                  </button>
                ))}
              </div>
            )}
      ...
      {section === 'map' && (
        <AdminRestaurantMap
          ...
          onBookingAction={runBookingAction}
          onCreateBooking={openManual}
          onChangeBookingTable={changeBookingTableTo}
        />
      )}
      ...
      {section === 'tables' && (
        <section className="mt-3 space-y-3">
          ...
          {groupedTables.map(([location, locationTables]) => {
            ...
            return (
              <div key={location} className="rounded-[24px] border border-white/10 bg-neutral-950 p-3">
                <button type="button" onClick={() => setOpenLocation(opened ? null : location)} className="flex w-full items-center justify-between gap-3 text-left">
                  <div><h3 className="text-lg font-black">{location}</h3><p className="mt-1 text-xs text-white/40">{locationTables.length} столів · {busyCount} зі статусом</p></div>
                  <span className="rounded-2xl border border-cyan-200/25 bg-cyan-300/5 px-3 py-2 text-sm font-black text-cyan-100">{opened ? 'Сховати' : 'Відкрити'}</span>
                </button>
                {opened && (
                  <div className="mt-3 grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
                    {locationTables.map((table) => {
                      ...
                      return (
                        <div key={table.id} className="rounded-2xl border border-white/10 bg-black/35 p-3">
                          <div className="flex items-center justify-between gap-2">
                            <p className="font-black">Стіл №{table.tableNumber}</p>
                            <span className={`rounded-full border border-white/10 bg-black/30 px-2 py-1 text-[10px] font-bold ${table.status === 'free' ? 'text-white/45' : ''}`} style={table.status === 'free' ? undefined : { color: TABLE_STATUS_COLOR[table.status] }}>{TABLE_STATUS_LABEL[table.status]}</span>
                          </div>
                          ...
                          {expanded && (
                            <div className="mt-2 grid grid-cols-2 gap-2">
                              ...
                              {table.status === 'closed'
                                ? <NeonButton compact tone="green" busy={busy === `table:${table.id}:open`} disabled={Boolean(busy)} onClick={() => runTableAction(table, 'open')}>Відкрити</NeonButton>
                                : <NeonButton compact tone="red" busy={busy === `table:${table.id}:close`} disabled={Boolean(busy)} onClick={() => runTableAction(table, 'close')}>Закрити</NeonButton>}
                            </div>
                          )}
                        </div>
                      );
      ...
      {section === 'blacklist' && (
        <section className="mt-3 space-y-3">
          ...
          {blacklisted.map((client) => (
            <div key={client.id} className="rounded-[22px] border border-red-300/20 bg-neutral-950 p-4">
              <div className="flex items-center justify-between gap-3">
                <div className="min-w-0"><p className="truncate text-lg font-black">{client.fullName}</p><a href={`tel:${normalizePhone(client.phone)}`} className="mt-1 block text-sm text-amber-100">{client.phone}</a></div>
                <NeonButton compact tone="green" busy={busy === `client:${client.id}:unblacklist`} disabled={Boolean(busy)} onClick={() => toggleBlacklist(client, false)}>Вилучити</NeonButton>
              </div>
            </div>
          ))}
