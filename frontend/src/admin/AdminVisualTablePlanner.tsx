...
export default function AdminVisualTablePlanner({
  ...
  async function createManualBooking() {
    ...
    const phone = rawPhone ? normalizeUkrainePhone(rawPhone) : null;
    const guestsCount = Number(manualGuestsCount);
    const durationMinutes = Number(manualDurationMinutes);

    if (!fullName || !manualDate || !manualTime) {
      setError('Заповніть ім’я, дату та час');
      return;
    }
    ...
    if (rawPhone && !phone) {
      setError('Вкажіть телефон у форматі +380 (XX) XXX-XX-XX або залиште поле порожнім');
      return;
    }
