...
import FutureAvailabilityPanel from '../admin/FutureAvailabilityPanel';
...
function statusTone(status: Restaurant['status'] | undefined): string {
...
export default function CompactDirectorPanel() {
  ...
  async function load(silent = false) {
    ...
    if (staffResult.status === 'fulfilled') setStaff(unwrap<StaffMember[]>(staffResult.value));
    if (reviewsResult.status === 'fulfilled') setReviews(unwrap<GuestReviewRecord[]>(reviewsResult.value));
    if (logsResult.status === 'fulfilled') setLogs(unwrap<LogRecord[]>(logsResult.value));
  ...
  async function toggleBlacklist(client: Client) {
    ...
    try {
      if (client.isBlacklisted) await clientsApi.unblacklist(client.id);
      else await clientsApi.blacklist(client.id);
  ...
  return (
    <div className="min-h-screen bg-black pb-28 text-white">
      ...
      <main className="mx-auto max-w-7xl space-y-4 p-3 sm:p-5 lg:p-6">
        ...
        {tab === 'overview' && (
          <>
            <section className="rounded-[26px] border border-amber-200/25 bg-[radial-gradient(circle_at_top_right,rgba(251,191,36,.12),transparent_45%)] p-4 shadow-[0_0_42px_rgba(251,191,36,.08)] sm:p-5">
              <div className="flex items-center justify-between gap-3">
                <div>
                  <p className="text-xs font-black uppercase tracking-[0.2em] text-amber-100/55">Потребує уваги</p>
                  <h1 className="mt-1 text-2xl font-black">Операційний центр</h1>
                </div>
                <span className={`rounded-full border px-3 py-1.5 text-xs font-bold ${statusTone(restaurant?.status)}`}>{statusText(restaurant?.status)}</span>
              </div>
              <div className="mt-4 grid gap-2 sm:grid-cols-2 lg:grid-cols-4">
                {attention.length ? attention.map((item) => <Attention key={item.title} {...item} />) : <div className="rounded-2xl border border-emerald-300/25 bg-emerald-400/10 p-4 text-emerald-100 sm:col-span-2 lg:col-span-4"><ShieldCheck size={20} /><p className="mt-2 font-black">Усе спокійно</p></div>}
              </div>
            </section>

            <section className="grid grid-cols-2 gap-2 lg:grid-cols-4">
              <Metric label="Очікують рішення" value={stats.pending} tone="amber" />
              <Metric label="Гостей очікується" value={stats.guests} tone="violet" />
              <Metric label="Столів зайнято" value={stats.occupied} tone="red" />
              <Metric label="Працівників на зміні" value={stats.onShift} tone="cyan" />
            </section>

            <section className="grid gap-3 lg:grid-cols-[1.15fr_.85fr]">
              <GlassCard title="Найближчі бронювання" eyebrow={dateLabel(selectedDate)}>
                <div className="space-y-2">
                  {filteredBookings.slice(0, 5).map((booking) => <BookingLine key={booking.id} booking={booking} />)}
                  {!filteredBookings.length && <Empty text="Бронювань немає" />}
                </div>
              </GlassCard>
              <GlassCard title="Швидке керування" eyebrow="Ресторан">
                <div className="grid gap-2">
                  <Action label="Відкрити ресторан" tone="green" disabled={Boolean(busy)} onClick={() => void runRestaurant('open')} />
                  <Action label="Закрити онлайн-бронювання" tone="amber" disabled={Boolean(busy)} onClick={() => void runRestaurant('closeBooking')} />
                  <Action label="Закрити ресторан" tone="red" disabled={Boolean(busy)} onClick={() => void runRestaurant('close')} />
                </div>
              </GlassCard>
            </section>
          </>
        ...
        {tab === 'bookings' && (
          <section className="space-y-3">
            <Heading title="Бронювання" subtitle="Сьогодні, майбутні дати та архів через календар." />
            <div className="grid gap-2 rounded-[22px] border border-white/10 bg-neutral-950 p-3 sm:grid-cols-[auto_1fr]">
              <input type="date" value={selectedDate} onChange={(event) => setSelectedDate(event.target.value)} className="h-12 rounded-2xl border border-white/10 bg-black/40 px-4 text-sm [color-scheme:dark]" />
              <label className="relative">
                <Search size={17} className="absolute left-4 top-1/2 -translate-y-1/2 text-white/35" />
                <input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Ім’я, телефон, стіл або локація" className="h-12 w-full rounded-2xl border border-white/10 bg-black/40 pl-11 pr-4 text-sm outline-none focus:border-amber-200/40" />
              </label>
            </div>
            <div className="flex gap-2 overflow-x-auto pb-1">
              {([['all','Усі'],['pending','Очікують'],['approved','Підтверджені'],['no_show','Гість не прийшов'],['completed','Завершені'],['cancelled','Скасовані']] as Array<[BookingFilter,string]>).map(([value,label]) => <button key={value} type="button" onClick={() => setBookingFilter(value)} className={`whitespace-nowrap rounded-2xl border px-3 py-2.5 text-xs font-bold ${bookingFilter === value ? 'border-amber-200/50 bg-amber-300/15 text-amber-100' : 'border-white/10 bg-white/[0.03] text-white/50'}`}>{label}</button>)}
            </div>
            <div className="space-y-2">{filteredBookings.map((booking) => <BookingLine key={booking.id} booking={booking} detailed />)}{!filteredBookings.length && <Empty text="Бронювань не знайдено" />}</div>
          </section>
        )}

        {tab === 'locations' && (
          <section className="space-y-3">
            <Heading title="Локації" subtitle="Компактний стан усіх зон без довгої стрічки столів." />
            <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
              {locations.map((location) => (
                <article key={location.key} className="rounded-[22px] border border-white/10 bg-neutral-950 p-4">
                  <div className="flex items-start justify-between gap-3"><div><div className="flex items-center gap-2"><span className={`h-2.5 w-2.5 rounded-full ${location.zone?.isClosed ? 'bg-red-500' : 'bg-emerald-400 shadow-[0_0_10px_rgba(52,211,153,.7)]'}`} /><h3 className="font-black">{location.label}</h3></div><p className="mt-2 text-xs text-white/40">{location.bookings} бронювань · {location.occupied} зайнято · {location.cleaning} готується</p></div><Armchair size={19} className="text-white/30" /></div>
                  {location.zone && <button type="button" disabled={Boolean(busy)} onClick={() => void changeZone(location.zone as Zone, !location.zone?.isClosed)} className={`mt-3 w-full rounded-2xl border px-3 py-2.5 text-xs font-black ${location.zone.isClosed ? 'border-emerald-300/35 bg-emerald-400/10 text-emerald-100' : 'border-red-300/30 bg-red-500/10 text-red-100'}`}>{location.zone.isClosed ? 'Відкрити локацію' : 'Закрити локацію'}</button>}
                </article>
              ))}
            </div>
            <button type="button" onClick={() => setPlannerOpen(true)} className="flex w-full items-center justify-between rounded-[22px] border border-fuchsia-200/30 bg-fuchsia-400/10 p-4 text-left text-fuchsia-100 shadow-[0_0_26px_rgba(217,70,239,.08)]"><div><p className="font-black">Планування на майбутні дати</p><p className="mt-1 text-xs text-white/45">Закриття столів і локацій за датою та часом</p></div><CalendarClock size={21} /></button>
          </section>
        )}

        {tab === 'guests' && (
          <section className="space-y-3">
            <Heading title="База гостей" subtitle="Відвідування, чорний список і ручна розсилка." />
            <div className="grid gap-2 sm:grid-cols-[1fr_auto]">
              <label className="relative"><Search size={17} className="absolute left-4 top-1/2 -translate-y-1/2 text-white/35" /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Ім’я або телефон" className="h-12 w-full rounded-2xl border border-white/10 bg-neutral-950 pl-11 pr-4 text-sm outline-none focus:border-violet-200/40" /></label>
              <button type="button" onClick={() => setBroadcastOpen(true)} className="rounded-2xl border border-violet-200/35 bg-violet-400/10 px-5 py-3 text-sm font-black text-violet-100 shadow-[0_0_24px_rgba(167,139,250,.09)]"><Send size={16} className="mr-2 inline" />Обрати гостей</button>
            </div>
            <div className="space-y-2">{filteredClients.map((client) => <article key={client.id} className={`rounded-[22px] border p-4 ${client.isBlacklisted ? 'border-red-300/30 bg-red-500/[0.07]' : 'border-white/10 bg-neutral-950'}`}><div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><div className="flex flex-wrap items-center gap-2"><h3 className="font-black">{client.fullName}</h3>{client.isRegular && <Badge text="Постійний гість" tone="violet" />}{client.isBlacklisted && <Badge text="Чорний список" tone="red" />}</div><p className="mt-1 text-sm text-amber-100">{client.phone}</p><p className="mt-2 text-xs text-white/40">{client.visitsCount} відвідувань · {client.totalGuests} гостей загалом</p></div><button type="button" disabled={Boolean(busy)} onClick={() => void toggleBlacklist(client)} className={`rounded-2xl border px-3 py-2.5 text-xs font-black ${client.isBlacklisted ? 'border-emerald-300/35 bg-emerald-400/10 text-emerald-100' : 'border-red-300/30 bg-red-500/10 text-red-100'}`}>{client.isBlacklisted ? 'Розблокувати' : 'Заблокувати'}</button></div></article>)}{!filteredClients.length && <Empty text="Гостей не знайдено" />}</div>
          </section>
        )}

        {tab === 'team' && (
          <section className="space-y-3">
            <Heading title="Команда" subtitle="Хто зараз на зміні та які права має Адміністратор." />
            <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">{staff.filter((member) => !member.isArchived).map((member) => <article key={member.id} className="rounded-[22px] border border-white/10 bg-neutral-950 p-4"><div className="flex items-center justify-between gap-3"><div><p className="font-black">{member.fullName}</p><p className="mt-1 text-xs text-white/40">{member.role === 'owner' ? 'Директор' : member.role === 'admin' ? 'Адміністратор' : member.role === 'waiter' ? 'Офіціант' : 'Кальянник'}</p></div><span className={`h-3 w-3 rounded-full ${member.isOnShift ? 'bg-emerald-400 shadow-[0_0_12px_rgba(52,211,153,.7)]' : 'bg-white/20'}`} /></div><p className="mt-3 text-xs text-white/45">{member.isOnShift ? `На зміні${member.shiftStartedAt ? ` з ${new Date(member.shiftStartedAt).toLocaleTimeString('uk-UA',{hour:'2-digit',minute:'2-digit'})}` : ''}` : 'Не на зміні'}</p></article>)}</div>
            <GlassCard title="Права Адміністратора" eyebrow="Директор керує доступом">
              <div className="grid gap-2 sm:grid-cols-2">
                <RightToggle label="Керувати локаціями" value={adminRights.zones} onChange={(value) => setAdminRights((current) => ({ ...current, zones: value }))} />
                <RightToggle label="Керувати онлайн-бронюванням" value={adminRights.onlineBooking} onChange={(value) => setAdminRights((current) => ({ ...current, onlineBooking: value }))} />
        ...
        {tab === 'site' && (
          <section className="space-y-3">
            <Heading title="Сайт" subtitle="Режим оформлення та робочі повідомлення без зміни SVG і карт столів." />
            <GlassCard title="Оформлення" eyebrow="День · Ніч · Свято"><div className="grid grid-cols-3 gap-2">{(['day','night','holiday'] as SiteMode[]).map((mode) => <button key={mode} type="button" onClick={() => void changeMode(mode)} disabled={Boolean(busy)} className={`rounded-2xl border px-3 py-4 text-sm font-black ${restaurant?.siteMode === mode ? 'border-amber-200/55 bg-amber-300/15 text-amber-100 shadow-[0_0_24px_rgba(251,191,36,.12)]' : 'border-white/10 bg-white/[0.03] text-white/50'}`}>{mode === 'day' ? 'День' : mode === 'night' ? 'Ніч' : 'Свято'}</button>)}</div></GlassCard>
            <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3"><SystemCard title="Титульна ротація" status="Працює" text="Фотографії змінюються окремо від режиму сайту." tone="green" /><SystemCard title="POS" status="Не підключено" text="Інтеграція ще не реалізована." tone="neutral" /><SystemCard title="Expz" status="Не підключено" text="Є лише посилання на меню, API не підключено." tone="neutral" /></div>
          </section>
        )}
      ...
      {plannerOpen && <FutureAvailabilityPanel onClose={() => setPlannerOpen(false)} />}
      {broadcastOpen && <BroadcastModal clients={filteredClients} text={broadcastText} setText={setBroadcastText} selected={selectedClients} setSelected={setSelectedClients} all={sendToAll} setAll={setSendToAll} busy={busy === 'broadcast'} onClose={() => setBroadcastOpen(false)} onSend={() => void sendBroadcast()} />}
    </div>
  );
}
...
function SystemCard({ title, status, text, tone }: { title: string; status: string; text: string; tone: 'green' | 'amber' | 'red' | 'neutral' }) {
  const style = tone === 'green' ? 'border-emerald-300/25 bg-emerald-400/[0.06]' : tone === 'amber' ? 'border-amber-200/25 bg-amber-300/[0.06]' : tone === 'red' ? 'border-red-300/25 bg-red-500/[0.06]' : 'border-white/10 bg-neutral-950';
