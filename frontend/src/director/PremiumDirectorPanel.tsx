...
export default function PremiumDirectorPanel() {
  ...
  function openNotice(item: NoticeItem) {
    ...
    window.setTimeout(() => {
      document.getElementById('director-reviews')?.scrollIntoView({ behavior: 'smooth', block: 'start' });
    }, 0);
  }
  ...
  return (
    <div className="min-h-screen bg-[#030403] pb-28 text-white [background-image:radial-gradient(circle_at_12%_0%,rgba(52,211,153,.055),transparent_30%),radial-gradient(circle_at_88%_8%,rgba(251,191,36,.07),transparent_28%)]">
      ...
      <main className="mx-auto max-w-7xl space-y-4 p-3 sm:p-5 lg:p-6">
        ...
        {tab === 'stats' && <section className="space-y-3"><Heading title="Статистика" subtitle="Відвідуваність, бронювання та завантаженість ресторану — без грошових сум." /><PremiumCard className="grid gap-2 sm:grid-cols-[auto_1fr]"><input type="date" value={selectedDate} onChange={(event) => setSelectedDate(event.target.value)} className="h-12 rounded-2xl border border-white/12 bg-black/45 px-4 text-sm [color-scheme:dark] outline-none focus:border-amber-100/45" /><p className="self-center text-sm text-white/45">Оберіть дату статистики</p></PremiumCard><section className="grid grid-cols-2 gap-2 lg:grid-cols-4"><Metric label="Бронювань сьогодні" value={todayAnalytics?.bookingsCount || 0} tone="gold" /><Metric label="Гостей сьогодні" value={todayAnalytics?.guestsCount || 0} tone="violet" /><Metric label="Зайнятих столів" value={todayAnalytics?.occupiedTables || 0} tone="red" /><Metric label="Вільних столів" value={todayAnalytics?.freeTables || 0} tone="cyan" /></section><PremiumCard><Eyebrow>{dateLabel(selectedDate)}</Eyebrow><h2 className="mt-1 text-xl font-black">Завантаженість ресторану за годинами</h2><p className="mt-2 text-sm text-white/45">Кількість бронювань і гостей. Фінансові показники не відображаються.</p><div className="mt-4 space-y-2">{hourlyEntries.map(([hour, load]) => <div key={hour} className="grid grid-cols-[64px_1fr_auto] items-center gap-3 rounded-2xl border border-cyan-200/20 bg-black/35 p-3"><span className="font-mono font-black text-cyan-100">{hour}</span><span className="h-2 overflow-hidden rounded-full bg-white/10"><span className="block h-full rounded-full bg-cyan-200 shadow-[0_0_14px_rgba(103,232,249,.7)]" style={{ width: `${Math.min(100, Math.max(8, load.bookingsCount * 12))}%` }} /></span><span className="text-xs text-white/55">{load.bookingsCount} броней · {load.guestsCount} гостей</span></div>)}{!hourlyEntries.length && <Empty text="На цю дату навантаження ще немає" />}</div></PremiumCard></section>}
      ...
      {notificationsOpen && <NotificationsDrawer items={attention} onClose={() => setNotificationsOpen(false)} onItemClick={openNotice} />}
...
function Attention({ item, onClick }: { item: NoticeItem; onClick?: () => void }) {
  ...
  const content = <><CircleAlert size={18} /><p className="mt-2 font-black">{item.title}</p><p className="mt-1 text-xs text-white/45">{item.text}</p></>;
  if (onClick) {
    return <button type="button" onClick={onClick} className={`w-full rounded-2xl border bg-black/40 p-4 text-left transition hover:bg-white/[0.025] active:scale-[.99] ${style}`}>{content}</button>;
  }
  return <div className={`rounded-2xl border bg-black/40 p-4 ${style}`}>{content}</div>;
}
...
function NotificationsDrawer({ items, onClose, onItemClick }: { items: NoticeItem[]; onClose: () => void; onItemClick: (item: NoticeItem) => void }) {
  return <div className="fixed inset-0 z-[90] bg-black/75 p-3 backdrop-blur-xl" onMouseDown={onClose}><aside className="ml-auto h-full w-full max-w-md overflow-y-auto rounded-[28px] border border-rose-100/25 bg-[#050505]/95 p-4 shadow-[0_0_70px_rgba(244,63,94,.12)]" onMouseDown={(event) => event.stopPropagation()}><div className="flex items-center justify-between"><div><Eyebrow>Центр подій</Eyebrow><h2 className="mt-1 text-2xl font-black">Повідомлення</h2></div><IconButton label="Закрити" onClick={onClose}><X size={18} /></IconButton></div><div className="mt-5 space-y-2">{items.map((item) => <Attention key={item.id} item={item} onClick={directorNoticeDestination(item.id) ? () => onItemClick(item) : undefined} />)}{!items.length && <Empty text="Нових подій немає" />}</div></aside></div>;
}
