...
export class Booking {
  ...
  /** Ідентифікатор браузера гостя зберігається лише як SHA-256 hash. */
  @Index()
  ...
  @Column({ name: 'lateness_hours', type: 'integer', nullable: true })
  latenessHours: number | null;

  @Column({ name: 'lateness_minutes', type: 'integer', nullable: true })
  latenessMinutes: number | null;

  @Column({ name: 'expected_arrival_at', type: 'timestamp', nullable: true })
  expectedArrivalAt: Date | null;

  @Column({ name: 'guest_notification', type: 'jsonb', nullable: true })
  guestNotification: GuestBookingNotification | null;

  @Column({ name: 'manual_change_reason', type: 'text', nullable: true })
  manualChangeReason: string | null;

  @Column({ name: 'manually_changed_at', type: 'timestamp', nullable: true })
