...
function loadMigrations(require) {
  const { InitialSchemaBaseline2026081300000 } = require('../dist/migrations/2026081300000-InitialSchemaBaseline.js');
  const { CreateStaffPinAttempts2026081400010 } = require('../dist/migrations/2026081400010-CreateStaffPinAttempts.js');
  const { UpgradeStaffPinAttemptsPerAttempt2026081400020 } = require('../dist/migrations/2026081400020-UpgradeStaffPinAttemptsPerAttempt.js');
  const { CreateWaiterCalls2026081500010 } = require('../dist/migrations/2026081500010-CreateWaiterCalls.js');
  const { AddWaiterCallAssignmentActive2026081500015 } = require('../dist/migrations/2026081500015-AddWaiterCallAssignmentActive.js');
  const { CloseInactiveWaiterCalls2026081500020 } = require('../dist/migrations/2026081500020-CloseInactiveWaiterCalls.js');
  const { AddGuestReviewArchive2026082200010 } = require('../dist/migrations/2026082200010-AddGuestReviewArchive.js');
  const { AddLogArchive2026082400010 } = require('../dist/migrations/2026082400010-AddLogArchive.js');
  const { AddManualBookingGuestName2026082400020 } = require('../dist/migrations/2026082400020-AddManualBookingGuestName.js');
  const { AddTableWaiterOwnership2026091901000 } = require('../dist/migrations/2026091901000-AddTableWaiterOwnership.js');
...
async function assertNoBaselineObjects(dataSource) {
  ...
  `);
  if (extension?.present) throw new Error('Fresh baseline revert left uuid-ossp installed');
}
...
export async function runFreshSchemaBaselineValidation(mode, env = process.env) {
  ...
  if (!['apply', 'revert'].includes(mode)) throw new Error('Mode must be apply or revert');
  ...
  try {
    if (mode === 'apply') {
      ...
      assertMigrationNames(
        applied.map((migration) => migration.name), EXPECTED_FRESH_BASELINE_HISTORY,
        'fresh baseline application',
      );
      assertMigrationNames(
        await readMigrationHistory(dataSource), EXPECTED_FRESH_BASELINE_HISTORY,
    ...
    assertMigrationNames(await readMigrationHistory(dataSource), [], 'post-revert history');
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  main().catch((error) => {
    console.error(`Fresh schema baseline failed: ${error instanceof Error ? error.message : String(error)}`);
    process.exitCode = 1;
  });
}
