...
test('completion never claims table is free if another checked-in booking exists', async () => {
  const writes = [];
  const table = { id: 'table-8', status: 'occupied' };
  const booking = {
    id: 'booking-8', bookingDate: kyivToday(), status: 'approved',
    checkedInAt: new Date(), table, client: null,
  };
  const repos = {
    Booking: {
    ...
    BookingHistory: {
      async findOne() {
        return { action: 'booking_checked_in', actorRole: 'waiter', actorStaffId: 'serhii' };
      },
      create(value) { return value; },
      async save() { writes.push('history.save'); },
    },
  };
  const manager = { getRepository(entity) { return repos[entity.name]; } };
  const dataSource = { getRepository: manager.getRepository, transaction: async (job) => job(manager) };
  const raw = {
  ...
  await assert.rejects(
    service.complete(booking.id, { role: 'waiter', staffId: 'serhii' }),
