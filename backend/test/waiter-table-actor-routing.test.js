...
test('site table endpoints pass authenticated actor to guarded service', async () => {
  const calls = [];
  const tables = {
...
test('Telegram booking cleaning passes accepting waiter identity', async () => {
  const { menu, calls } = menuHarness();

  assert.equal(
    await menu.handle('booking_cleaning', 'booking-8', 'chat', actor),
    true,
  );
...
test('other waiter cannot invoke stale Telegram cleaning callback', async () => {
  const { menu, calls } = menuHarness();
