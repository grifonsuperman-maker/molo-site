...
test('admin reschedule rejection reuses the same BookingsService path as Telegram', async () => {
  const calls = [];
  const bookings = {
...
test('web reschedule endpoints inherit Admin and Director access from the controller', () => {
  assert.deepEqual(Reflect.getMetadata(ROLES_KEY, AdminAttentionController), [
    'owner',
    'admin',
  ]);
  ...
  for (const methodName of [
    ...
    assert.equal(
      ...
      undefined,
      `${methodName} must inherit owner/admin access instead of narrowing Director permissions`,
    );
  }
});
